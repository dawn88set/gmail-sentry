...
const state = () => {
  ...
  return {
    ...
    // NOT a health signal, despite the name. containerId is null on every app
    // in this account including ones demonstrably serving traffic, so an empty
    // value says nothing about whether the deploy runs. Kept only so the log
